...
namespace RTGui
{
    ...
    public class ClassifierAdapter
    {
        ...
        public ClassifierAdapter(int modeCount, int inputSize = 100)
        {
            InputSize = inputSize;
            ModeCount = modeCount;
        ...
        public void SetupNetwork(NeuralNetworkType type)
        {
            ...
            if (type == NeuralNetworkType.BackPropagating)
                _classifier.CreateFixedSizeNetwork(InputSize, InputSize / 2, ModeCount, 3); // 2 hidden layers + output layer
            else if (type == NeuralNetworkType.CascadeCorrelation)
                _classifier.CreateCascadeNetwork(InputSize, ModeCount);
            _ready = true;
        }
        ...
        private async void OnSampleAnalysed(IHilbertSpectrum hs, int channel)
        {
            if (_mode == -1) {
                return;
            }
            ...
            double interval = (hs.MaxFrequency - hs.MinFrequency) / InputSize;
            for (double w = hs.MinFrequency; w <= hs.MaxFrequency; w += interval) {
            ...
            if (channel == 7) {
                ...
                if (_mode == -2) {
                    ...
                    await _classifier.ClassifyAsync(_inputData.ToArray(), output);
